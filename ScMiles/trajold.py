...
#        sys.exit()
    ...
    def check_finished(self, new_path):
        if os.path.isfile(new_path + '/' + self.parameter.outputname + '.colvars.traj') and self.parameter.software == 'gromacs':
            return True
    ...
    def launch(self, joblist=None, lastLog=None):
            ...
            if lastLog == "Finished running trajectories":
                ...
                for j in range(self.parameter.trajPerLaunch):
                    ...
                    if self.parameter.software == 'namd':
                        ...
                        plumed(self.parameter, free='yes', anchor1=anchor1, anchor2=anchor2).generate_plumed()
        ...
        ms_copy = self.parameter.MS_list.copy()
        for name in ms_copy:
            #break
        ...
        for name in ms_copy:
            ...
            if self.parameter.software == 'gromacs':
                time, final_ms = plumed(self.parameter, anchor1=anchor1, anchor2=anchor2, free='yes').final_ms_plumed()
            MSname = str(anchor1) + '_' + str(anchor2)
            MSpath = self.parameter.crdPath +  '/' + MSname
            last_frame = self.__snapshots(name)
    ...
    def find_new_milestones(self, last_frame, MSpath, new_milestones):                
        ...
        for i in range(1, last_frame):
            ...
            if not os.path.isfile(end_info):
                continue
            end = pd.read_csv(end_info, header=None, delimiter=r'\s+').values.tolist()[0]
            end_ms = str(end[0]) + '_' + str(end[1])
            if 'MS' + end_ms not in self.parameter.MS_list:
                new = True
                for item in new_milestones:
                    if end_ms in item[0]:
                        new = False
                        item[1] += 1
                if new == True:
                    new_milestones.append([end_ms, 1, MSpath + '/' + str(i)])
            #checking against user input
        print(new_milestones)
        #if self.parameter.new_ms_trajs != 0 and len(new_milestones) > 0:
        if len(new_milestones) > 0:
            for ms in new_milestones:
                if ms[1] >= self.parameter.new_ms_trajs:
                    if self.parameter.method == 1:
                        self.parameter.finished_constain.add(ms[0])
                    self.parameter.MS_list.add('MS' + end_ms)
                    skip_compute = True
                    if self.parameter.software == 'gromacs' and not os.path.isfile(self.parameter.crdPath + '/' + ms[0] + '/statefromseek.cpt'):
                        os.makedirs(self.parameter.crdPath + '/' + ms[0])
                        copy(ms[2] + '/state.cpt', self.parameter.crdPath + '/' + ms[0] + '/statefromseek.cpt')
